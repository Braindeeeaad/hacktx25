/**
 * Complete Mood System Test with Dummy Spending Data
 * Tests mood data + Gemini integration with realistic spending patterns
 * This shows the complete mood-enhanced analysis without needing Nessie API
 */
...
/**
 * Generate realistic spending data that correlates with mood patterns
 */
function generateRealisticSpendingData(moodEntries: Omit<MoodEntry, 'id' | 'userId'>[]) {
...
Analyze the correlation between mood patterns and spending behavior. Look for:
...
async function testCompleteMoodSystem() {
  console.log('🚀 COMPLETE MOOD SYSTEM TEST (WITH DUMMY SPENDING DATA)');
  console.log('=======================================================');
  console.log('🎯 Testing mood data + Gemini integration with realistic spending patterns');
  console.log('This shows the complete mood-enhanced analysis without needing Nessie API');

  const testUserId = 'single-user-system'; // Simulating single user setup
  const numDays = 30;

  try {
    // Step 1: Generate realistic mood data
    console.log('\n1️⃣ Generating realistic mood data...');
    const moodEntries = generateRealisticMoodData(testUserId, numDays);
    ...
    // Step 2: Generate realistic spending data that correlates with mood
    console.log('\n2️⃣ Generating realistic spending data with mood correlation...');
    const transactions = generateRealisticSpendingData(moodEntries);
    ...
    console.log('   ✅ Mood-correlated spending data generation');
