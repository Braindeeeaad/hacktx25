/**
 * Complete System Flow Test - Clean Version
 * Tests the full system: Cloudflare D1 → Nessie API → Gemini Analysis
 * Handles existing data and uses clean approach
 */
 ...
 */
async function testCompleteSystemClean() {
  console.log('🚀 COMPLETE SYSTEM FLOW TEST - CLEAN VERSION');
  console.log('==============================================');
  console.log('🎯 Testing full system: Cloudflare D1 → Nessie → Gemini Analysis');

  const testUserId = 'clean-system-test-user'; // Use different user ID to avoid conflicts
  const numDays = 30;
  const endDate = new Date();
  const startDate = new Date();
  ...
  try {
    // Step 1: Generate mood data (but don't save to Cloudflare to avoid conflicts)
    console.log('\n1️⃣ Generating mood data for analysis...');
    ...
    console.log('   ✅ Mood data generation and processing');
